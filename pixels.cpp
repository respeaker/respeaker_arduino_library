...
void Pixels::set_pixel(uint16_t index, uint32_t px_value) 
{
    if(index < leds_n) {
        uint8_t *ptr = pixels + index * 3;
        *ptr++ = ((px_value >> 8) & 0xff) * brightness;     // green
        *ptr++ = ((px_value >> 16) & 0xff) * brightness;    // red
        *ptr   = (px_value & 0xff) * brightness;            // blue
    }
}

void Pixels::set_pixel(uint16_t index, uint8_t r, uint8_t g, uint8_t b) 
{
    if(index < leds_n) {
        uint8_t *ptr = pixels + index * 3;
...
uint32_t Pixels::get_pixel(uint16_t index) 
{
    uint8_t *ptr = pixels + index * 3;
...
void Pixels::scroll(uint8_t size, uint32_t px_value, uint8_t time)
{
    ...
    for(int i = 1; i < leds_n + size; i++){
        if(i < size){
            for(int j = 0; j < i; j++){
                set_pixel(j, px_value);
            }
        }else if(i >= leds_n){
            for(int k = i - size; k < leds_n; k++){
                set_pixel(k, px_value);
            }
        }else{
            for(int m = i-size; m < i; m++){
                set_pixel(m, px_value);
            }
        }
        update();
        delay(time);
...
void Pixels::pass_by(uint8_t size, uint32_t px_value1, uint32_t px_value2, uint8_t time)
{
    ...
    for(int i = 1; i < leds_n + size; i++){
        if(i < size){
            for(int j = 0; j < i; j++){
                set_pixel(j, px_value1);
                set_pixel(leds_n -1 - j, px_value2);
            }
        }else if(i >= leds_n){
            for(int k = i - size; k < leds_n; k++){
                set_pixel(k, px_value1);
                set_pixel(leds_n -1 - k, px_value2);
            }
        }else{
            for(int m = i-size; m < i; m++){
                set_pixel(m, px_value1);
                set_pixel(leds_n -1 - m, px_value2);
            }
        }
        update();
        delay(time);
...
void Pixels::rainbow(uint8_t offset) 
{
    uint16_t i;
    for(i = 0; i < leds_n; i++) {
        set_pixel(i, wheel(((i * 256 / leds_n) + offset) & 255));
    }
    update();
}
